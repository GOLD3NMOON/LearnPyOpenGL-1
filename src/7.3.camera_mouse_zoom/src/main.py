...
firstMouse = True
yaw  = -90.0 # O yaw é inicializado em -90,0 graus, uma vez que um yaw de 0,0 resulta em um vetor de direção apontando para a direita; portanto, inicialmente rotacionamos um pouco para a esquerda.
pitch =  0.0
lastX =  800.0 / 2.0
lastY =  600.0 / 2.0
fov   =  45.0
...
def mouse_callback(window, xposIn, yposIn):
    ...
    sensitivity = 0.1 # altere este valor conforme sua preferência
    xoffset *= sensitivity
    yoffset *= sensitivity
    ...
    front.x = math.cos(glm.radians(pitch)) * math.cos(glm.radians(yaw))
    front.y = math.sin(glm.radians(pitch))
    front.z = math.cos(glm.radians(pitch)) * math.sin(glm.radians(yaw))
